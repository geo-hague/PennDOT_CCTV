...
function parseHighwayRef(ref) {
  if (!ref) return null;
  const m = ref.match(/^(I|US|PA)-(\d+)$/);
  if (!m) return null;
  return { type: m[1], number: parseInt(m[2], 10), isEven: parseInt(m[2], 10) % 2 === 0 };
}

// When multiple routes are concurrent (e.g. I-76 running with I-676 briefly
// near Philadelphia), PennDOT's mile markers and exit numbers follow the
...
// both should be treated as equally "primary", e.g. I-76/I-476) from a
// short accidental overlap with a lower-tier route. primaryHighwayRef()
// would arbitrarily tie-break two same-type refs by array order, which is
...
function shieldCandidates(parsed) {
  ...
  if (parsed.type === 'I') return [`I-${n}.svg`, `Interstate ${n}.svg`, `I-${n} (PA).svg`];
  if (parsed.type === 'US') return [`US ${n}.svg`, `US Highway ${n}.svg`, `US ${n} (PA).svg`];
  if (parsed.type === 'PA') return [`PA-${n}.svg`, `Pennsylvania Route ${n}.svg`, `PA ${n}.svg`];
  return [];
}
...
// ---------- Mile marker lookup ----------
// PennDOT's Interstate Mile Markers layer is scoped to interstates only
// (per its name and confirmed by inspection), so route matching is
// simpler than DE/MD: only ever match when the current ref is type "I",
// and just compare the numeric route number. ST_RT_NO is zero-padded
// (e.g. "0095" for I-95) — strip the leading zeros. MILE_MARKER is a
// clean numeric field (e.g. 16.00), unlike DE's LEGEND_TEXT quirks — no
// special parsing needed. No direction field was found in this layer
// (same situation as MD), so direction comes from bearing alone.
function paRouteMatches(attrs, parsedRef) {
  if (!attrs || !parsedRef) return false;
  if (parsedRef.type !== 'I') return false; // this layer only covers interstates
  const stripped = String(attrs.ST_RT_NO || '').replace(/^0+/, '');
  return Number(stripped) === parsedRef.number;
}

function parsePaMilemarkerMp(mileMarker) {
  const n = parseFloat(mileMarker);
...
async function queryNearestMileMarkers(lat, lon) {
  ...
  const params = new URLSearchParams({
    ...
    outFields: 'ST_RT_NO,MILE_MARKER',
    outSR: '4326',          // otherwise geometry comes back in Web Mercator meters, not lat/lon
    returnGeometry: 'true', // no confirmed flat lat/lon attribute fields — geometry.x/y instead
  });
  try {
...
async function updateMilepostAndDirection(lat, lon) {
  ...
  const withMeta = features.map(f => {
    ...
    const mp = parsePaMilemarkerMp(a.MILE_MARKER);
    const geom = f.geometry || {};
    const dist = (geom.y != null && geom.x != null)
      ? haversineMeters(lat, lon, geom.y, geom.x) // geometry.y = lat, geometry.x = lon (outSR=4326)
      : Infinity;
    return { mp, dist, lat: geom.y, lon: geom.x, attrs: a, direction: null }; // no direction field found in this layer
  });
  ...
  for (const ref of tryRefs) {
    ...
    const routeCandidates = withMeta.filter(f => f.mp != null && paRouteMatches(f.attrs, parsedForRef));
    if (!routeCandidates.length) continue;

    // Reject opposite-carriageway markers whenever we know our direction
    // and the marker declares its own. PennDOT's Interstate Mile Markers
    // layer has no per-marker direction field at all (confirmed field
    // list: ST_RT_NO, MILE_MARKER — same situation as MD), so f.direction
    // is always null and every marker falls through to "no direction data,
    // keep everything" here. Direction comes from bearing alone
    // (bearingDirection/highwayDirectionLabel below) instead.
  ...
  if (!matched) {
    setDebug({ milepostLookup: 'no matching-route markers in our direction of travel', sampleRouteAttrs: features.slice(0, 5).map(f => f.attributes && { stRtNo: f.attributes.ST_RT_NO, mileMarker: f.attributes.MILE_MARKER }) });
    return;
  }
